...
namespace Footsteps
{
    ...
    public class CharacterFootsteps : MonoBehaviour
    {
        ...
        private bool DontUseUpdate = true;
        ...
        void CheckGround()
        {
            ...
            int newSurfaceIndex = SurfaceManager.singleton.GetSurfaceIndex(currentGroundInfo.collider, currentGroundInfo.point);
            if(newSurfaceIndex != SurfaceIndex)
            {
                SurfaceIndex = newSurfaceIndex;
                GroundChanged(newSurfaceIndex);
            }
        ...
        private void GroundChanged(int groundIndex)
        {
            //Change speed
            float speed;
            //Change animation
            ...
            switch (groundIndex)
            {
                case 5: speed = 4.8f; animationId = 0; break;    //Grass
                case 6: speed = 6; animationId = 1; break;    //Sand
                default: return;
            }

            this.GetComponent<SimpleCharacterControlFree>().MoveSpeed = speed;

            foreach(Animator animator in animators)
            {
                animator.runtimeAnimatorController = controllers[animationId];
            }
            foreach(GameObject particle in particles)
            {
                particle.SetActive(animationId == 0);
            }
        }
    }
}
